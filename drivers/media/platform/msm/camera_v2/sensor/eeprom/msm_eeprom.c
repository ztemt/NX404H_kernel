...
int32_t read_eeprom_memory(struct msm_eeprom_ctrl_t *e_ctrl)
{
	...
	emap = eb_info->eeprom_map;
      #if 1 //add read sensor ID to check whehter I2C and power up ok.
	  ...
	  rc = e_ctrl->i2c_client.i2c_func_tbl->i2c_read(
	  	...
	  	CDBG("%s: read id: %x expected id :0x1C21 \n", __func__, chipid);
	  if (rc < 0) {
	  ...
	  if (chipid != 0x1C21) {
		  pr_err("read_eeprom_memory() chip id doesnot match\n");
	  }
	  
      #endif
	 #if 1  //jun modify for T4K37 OTP
		if (emap[0].pageen.valid_size) {  //write disable otp(0x3500=0x01)
		e_ctrl->i2c_client.addr_type = emap[0].pageen.addr_t;
		rc = e_ctrl->i2c_client.i2c_func_tbl->i2c_write(
			&(e_ctrl->i2c_client), emap[0].pageen.addr,
			1, emap[0].pageen.data_t);
		if (rc < 0) {
	...
	for (j = 0; j < eb_info->num_blocks; j++) {
		if (emap[j].page.valid_size) {  //write pagesel reg(0x3502=page)
		...
		if (emap[j].pageen.valid_size) { //write start otp access reg(0x3500=0x81)
		...
		if (emap[j].poll.valid_size) { //no need poll, so poll.valid_size=0
		...
		if (emap[j].mem.valid_size) {//read OTP data
			...
			memptr += emap[j].mem.valid_size;
             ...
             #if 0 //jun modify for T4K37 OTP
		if (emap[j].pageen.valid_size) {  //write disable otp 
			e_ctrl->i2c_client.addr_type = emap[j].pageen.addr_t;
			rc = e_ctrl->i2c_client.i2c_func_tbl->i2c_write(
				&(e_ctrl->i2c_client), emap[j].pageen.addr,
			...
			if (rc < 0) {
             ...
             #if 1  //jun modify for T4K37 OTP
		if (emap[0].pageen.valid_size) {  //write disable otp(0x3500=0) 
		e_ctrl->i2c_client.addr_type = emap[0].pageen.addr_t;
		rc = e_ctrl->i2c_client.i2c_func_tbl->i2c_write(
			&(e_ctrl->i2c_client), emap[0].pageen.addr,
			0, emap[0].pageen.data_t);
		if (rc < 0) {
			pr_err("%s: page disable failed\n", __func__);
			return rc;
		}
...
int32_t msm_eeprom_platform_probe(struct platform_device *pdev)
{
	int32_t rc = 0;
	int32_t j = 0;
	uint32_t temp;
	...
	if (rc < 0) {
		...
		pr_err("%s line %d\n", __func__, __LINE__);
	for (j = 0; j < e_ctrl->num_bytes; j++)
		CDBG("page=%d,addr=%d,memory_data[%d] = 0x%X\n", j/64,j%64,j, e_ctrl->memory_data[j]);
      #if 0 //jun add
	rc = msm_camera_power_down(power_info, e_ctrl->eeprom_device_type,
		&e_ctrl->i2c_client);
	if (rc) {
...
//add by kwang
EXPORT_SYMBOL(msm_eeprom_platform_probe);
//end by kwang+
...
//static struct platform_driver msm_eeprom_platform_driver = {
 struct platform_driver msm_eeprom_platform_driver = {
	.driver = {
		.name = "qcom,eeprom",
		.owner = THIS_MODULE,
...
//add by kwang
EXPORT_SYMBOL(msm_eeprom_platform_driver);
//end by kwang
#if 0
