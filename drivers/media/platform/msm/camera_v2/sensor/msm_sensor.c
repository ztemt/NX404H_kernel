...
/*ZTEMT:Jinghongliang ADD For Read AF EEPROM -----Start*/
#ifdef CONFIG_IMX135
int temp_i2c_data_type_imx135 = 0;
bool af_read_eeprom_flag_imx135 = false;

unsigned short af_start_value_imx135_high = 0;
unsigned short af_start_value_imx135_low = 0;
unsigned short af_macro_value_imx135_high = 0;
unsigned short af_macro_value_imx135_low = 0;
unsigned short af_start_value_imx135 = 0;
unsigned short af_macro_value_imx135 = 0;
unsigned short module_id_imx135 = 0;

#define AF_MACRO_CURRENT_IMX135_LOW 	0x12
#define AF_MACRO_CURRENT_IMX135_HIGH 	0x13
#define AF_START_CURRENT_IMX135_LOW 	0x14
#define AF_START_CURRENT_IMX135_HIGH 	0x15
#endif
/*ZTEMT:Jinghongliang ADD For Read AF EEPROM -----End*/
...
int32_t msm_sensor_power_up(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	/* ZTEMT: Jinghongliang ADD for Read AF EEPROM ------Start*/
		...
		if(strcmp(data->sensor_name,"imx135") == 0){
			if(af_read_eeprom_flag_imx135 == false){
				af_read_eeprom_flag_imx135 = true;
				s_ctrl->sensor_i2c_client->cci_client->sid = 0xA0 >> 1;
				temp_i2c_data_type_imx135 = s_ctrl->sensor_i2c_client->addr_type;
				s_ctrl->sensor_i2c_client->addr_type = MSM_CAMERA_I2C_BYTE_ADDR;

				rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_read(
		                     s_ctrl->sensor_i2c_client,AF_START_CURRENT_IMX135_LOW,
					(uint16_t *)&af_start_value_imx135_low, MSM_CAMERA_I2C_BYTE_ADDR);
				//printk("AF EEPROM: af_start_value_imx135_low = 0x%x\n",af_start_value_imx135_low);

				rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_read(
		                     s_ctrl->sensor_i2c_client,AF_START_CURRENT_IMX135_HIGH,
					(uint16_t *)&af_start_value_imx135_high, MSM_CAMERA_I2C_BYTE_ADDR);
				//printk("AF EEPROM: af_start_value_imx135_high = 0x%x\n",af_start_value_imx135_high);

				rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_read(
		                     s_ctrl->sensor_i2c_client,AF_MACRO_CURRENT_IMX135_LOW,
					(uint16_t *)&af_macro_value_imx135_low, MSM_CAMERA_I2C_BYTE_ADDR);
				//printk("AF EEPROM: af_macro_value_imx135_low = 0x%x\n",af_macro_value_imx135_low);

				rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_read(
		                     s_ctrl->sensor_i2c_client,AF_MACRO_CURRENT_IMX135_HIGH,
					(uint16_t *)&af_macro_value_imx135_high, MSM_CAMERA_I2C_BYTE_ADDR);
				//printk("AF EEPROM: af_macro_value_imx135_high = 0x%x\n",af_macro_value_imx135_high);

				rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_read(
		                     s_ctrl->sensor_i2c_client,0x00,
					(uint16_t *)&module_id_imx135, MSM_CAMERA_I2C_BYTE_ADDR);
				//printk("AF EEPROM: moudle_id = %d\n",module_id_imx135);

				if(rc < 0){
					printk("AF EEPROM read Error!!\n");
					af_start_value_imx135 = 0;
					af_macro_value_imx135 = 0;
				}else{
					af_start_value_imx135 = (af_start_value_imx135_high << 8) | af_start_value_imx135_low;
					af_macro_value_imx135 = (af_macro_value_imx135_high << 8) | af_macro_value_imx135_low;
					//af_start_value_imx135 = 346;
					//af_macro_value_imx135 = 740;
					//printk("AF EEPROM: af_macro_value_imx135 = %d\n",af_macro_value_imx135);
					//printk("AF EEPROM: af_start_value_imx135 = %d\n",af_start_value_imx135);
				}
				s_ctrl->sensor_i2c_client->cci_client->sid = s_ctrl->sensordata->slave_info->sensor_slave_addr >> 1;
				s_ctrl->sensor_i2c_client->addr_type = temp_i2c_data_type_imx135;
				}
			}
	#endif
	/* ZTEMT: Jinghongliang ADD for Read AF EEPROM ------End*/
	...
	for (index--; index >= 0; index--) {
		...
		case SENSOR_VREG:
			...
			if(power_setting->seq_val==  CAM_VANA)
			{
				...
				gpio_free(data->gpio_conf->camera_avdd_enable_gpio);
			}
			else if(power_setting->seq_val==  CAM_VANA2)
			{
...
int32_t msm_sensor_platform_probe(struct platform_device *pdev, void *data)
{
	...
	#if  1//jun add for T4K37_OTP 
		if (strcmp(s_ctrl->sensordata->sensor_name,"t4k37_qtech_f4k37ab") == 0) {
		        printk("jun do nothing about power down\n");
		}
		else{
	               s_ctrl->func_tbl->sensor_power_down(s_ctrl);
		}
