...
#ifdef CONFIG_IMX135
extern unsigned short af_start_value_imx135;
extern unsigned short af_macro_value_imx135;
#endif
	...
	struct msm_actuator_set_info_t *set_info)
{
	...
	if (a_ctrl->step_position_table == NULL)
		return -ENOMEM;
	   // printk("af_start_value_imx135 = %d\n",af_start_value_imx135);
		//printk("af_macro_value_imx135 = %d\n",af_macro_value_imx135);
	if((set_info->actuator_params.i2c_addr == 0x18) && (af_start_value_imx135 > 0) && (af_macro_value_imx135 < 1024) \
		&& (af_macro_value_imx135 > af_start_value_imx135)){
		    if(af_start_value_imx135 > 355){
				cur_code = af_start_value_imx135-235;
			}else{
				if(af_start_value_imx135 > 250){
					cur_code = af_start_value_imx135-200;
				}else{
					cur_code = 30;
				}
			}
			...
			for (region_index = 0;region_index < a_ctrl->region_size;region_index++) {
				...
				if(region_index == 1){
					code_per_step = (af_macro_value_imx135 - af_start_value_imx135)*10;
