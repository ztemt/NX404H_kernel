...
#define PROX_THRESHOLD_DISTANCE             70
...
#define PROX_DATA_SAFE_RANGE_MAX            (PROX_DATA_TARGET + 100)
...
#define PROX_THRESHOLD_HIGH_MAX             600
#define PROX_THRESHOLD_HIGH_MIN             200
...
static void tmd2772_data_init(void)
{
    ...
    taos_datap->prox_thres_lo_min = PROX_THRESHOLD_HIGH_MIN;
...
static int taos_prox_offset_calculate(int data, int target)
{
    ...
    if (data > PROX_DATA_TARGET)
    {
		...
		if(data > 500)
		{
         	   offset = (data - PROX_DATA_TARGET) *10/88;
		}

		if(data > 700)
		{
         	   offset = (data - PROX_DATA_TARGET) *10/89;
		}	
		
		if(data > 900)
		{
         	   offset = (data - PROX_DATA_TARGET) *10/90;
		}
...
{  
    ...
    struct taos_prox_info *prox_cal_info = NULL;
        ...
        memset(prox_cal_info, 0, sizeof(struct taos_prox_info) * (PROX_OFFSET_CAL_BUFFER_SIZE));
	 ...
	 if ((ret = (i2c_smbus_write_byte_data(taos_datap->client, (TAOS_TRITON_CMD_REG|0x1E), 0))) < 0) {
